...
public:
  ...
  static constexpr uint32_t DEFAULT_OCTREE_DEBUG_DEPTH = 12;
  static constexpr uint32_t MAX_OCTREE_DEBUG_DEPTH = 32;
  ...
  std::vector<float> octreeLineData;
  Vec3 previewCenter;
  ...
  RenderMode renderMode{RenderMode::FullOctree};
  bool octreeRenderFinished{true};
  bool bruteForceRenderFinished{true};
  uint32_t octreeDebugDepth{DEFAULT_OCTREE_DEBUG_DEPTH};
  uint64_t octreeNextPixel{0};
  uint64_t bruteForceNextPixel{0};

  MyGLApp() : GLApp{600,600,1,"Octree Demo"} {}
  ...
  virtual void init() override {
    ...
    octreeLineData = scene.getOctreeLineData(octreeDebugDepth);
  ...
  void cycleRenderMode()
  {
    switch (renderMode) {
      case RenderMode::SplitScreen:
        renderMode = RenderMode::FullOctree;
        break;
      case RenderMode::FullOctree:
  ...
  void setOctreeDebugDepth(uint32_t debugDepth)
  {
    octreeDebugDepth = std::min(debugDepth, MAX_OCTREE_DEBUG_DEPTH);
    octreeLineData = scene.getOctreeLineData(octreeDebugDepth);
  }

  void increaseOctreeDebugDepth()
  {
    setOctreeDebugDepth(octreeDebugDepth + 1);
  }

	  void decreaseOctreeDebugDepth()
	  {
	    if (octreeDebugDepth > 0)
	      setOctreeDebugDepth(octreeDebugDepth - 1);
	  }
  ...
  void startRenderIfNeeded()
  {
    ...
    octreeRenderFinished = false;
    bruteForceRenderFinished = false;
    clearRenderImage();

    octreeRenderer.setCamera(camera);
    octreeRenderer.setScene(scene);
    octreeRenderer.setUseOctree(true);
    octreeRenderer.setScanDirection(ScanDirection::TopToBottom);
    bruteForceRenderer.setCamera(camera);
    bruteForceRenderer.setScene(scene);
    bruteForceRenderer.setUseOctree(false);
  ...
  void renderProgressiveStep()
  {
    ...
    switch (renderMode) {
      case RenderMode::SplitScreen: {
        ...
        renderPixelsForTime(octreeRenderer, 0, middle, octreeNextPixel, PIXELS_PER_BATCH, octreeRenderFinished);
        renderPixelsForTime(bruteForceRenderer, middle, image.width, bruteForceNextPixel, 1, bruteForceRenderFinished);
        break;
      }
      case RenderMode::FullOctree:
        renderPixelsForTime(octreeRenderer, 0, image.width, octreeNextPixel, PIXELS_PER_BATCH, octreeRenderFinished);
        break;
      case RenderMode::FullBruteForce:
  ...
  void drawPreviewText()
  {
    ...
    fontEngine->render("Octree Preview", getAspect(), 0.035f, {0.0f, -0.92f}, Alignment::Center, {1.0f, 0.0f, 0.0f, 0.9f});
  }

  void drawOctreeOverlay()
  {
    if (!showOctree || octreeLineData.empty())
      return;
    ...
    drawLines(octreeLineData, LineDrawType::LIST, 1.0f);
  ...
  virtual void keyboard(int key, int scancode, int action, int mods) override
  {
    ...
    switch (key) {
      ...
      case GLENV_KEY_RIGHT:
        increaseOctreeDebugDepth();
        break;
      case GLENV_KEY_LEFT:
        decreaseOctreeDebugDepth();
        break;
      case GLENV_KEY_UP:
        zoom(ZOOM_STEP);
        break;
