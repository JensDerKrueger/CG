...
public:
  ...
  double ddaMilliseconds{0.0};
  double bresenhamMilliseconds{0.0};
  double ddaMillisecondsTotal{0.0};
  double bresenhamMillisecondsTotal{0.0};
  uint64_t benchmarkSampleCount{0};
  size_t benchmarkLineCount{0};

  MyGLApp() : GLApp{800, 600, 1, "Exercise 14 - Line Rasterization"} {
  }
  ...
  std::string formatPerformanceText(const std::string& name, const double milliseconds) const {
    std::ostringstream text;
    text << name << ": " << std::llround(milliseconds)
         << " ms / " << benchmarkLineCount << " lines";
    return text.str();
  }

  void recordPerformanceSample(const double ddaSample, const double bresenhamSample) {
    ddaMillisecondsTotal += ddaSample;
    bresenhamMillisecondsTotal += bresenhamSample;
    ++benchmarkSampleCount;

    ddaMilliseconds = ddaMillisecondsTotal / double(benchmarkSampleCount);
    bresenhamMilliseconds = bresenhamMillisecondsTotal / double(benchmarkSampleCount);
  }

  void drawPerformanceText() {
    if (!fontEngine) return;

    const Vec4 textColor{0.08f, 0.09f, 0.11f, 0.92f};
    fontEngine->render(formatPerformanceText("DDA", ddaMilliseconds),
                       getAspect(), 0.028f, {-0.96f, -0.94f},
                       Alignment::Left, textColor);
    fontEngine->render(formatPerformanceText("Bresenham", bresenhamMilliseconds),
                       getAspect(), 0.028f, {0.96f, -0.94f},
                       Alignment::Right, textColor);
  }
  ...
  void renderLineSet() {
    ...
    std::vector<std::pair<PixelPoint, PixelPoint>> animatedLines;
    animatedLines.reserve(lines.size());

    for (size_t i = 0; i < lines.size(); ++i) {
      const PixelPoint a = animatedPoint(lines[i].first, i * 2);
      const PixelPoint b = animatedPoint(lines[i].second, i * 2 + 1);
      animatedLines.push_back({a, b});
    ...
    constexpr int benchmarkRepeats{512};
    benchmarkLineCount = animatedLines.size() * size_t(benchmarkRepeats);

    const auto ddaStart = Clock::now();
    for (int repeat = 0; repeat < benchmarkRepeats; ++repeat) {
      for (size_t i = 0; i < animatedLines.size(); ++i) {
        const PixelPoint a = animatedLines[i].first;
        const PixelPoint b = animatedLines[i].second;
        drawLineDDA(leftPoint(a.x, a.y), leftPoint(b.x, b.y), colors[i]);
      }
    }
    const double ddaSample =
      std::chrono::duration<double, std::milli>(Clock::now() - ddaStart).count();

    const auto bresenhamStart = Clock::now();
    for (int repeat = 0; repeat < benchmarkRepeats; ++repeat) {
      for (size_t i = 0; i < animatedLines.size(); ++i) {
        const PixelPoint a = animatedLines[i].first;
        const PixelPoint b = animatedLines[i].second;
    ...
    const double bresenhamSample =
      std::chrono::duration<double, std::milli>(Clock::now() - bresenhamStart).count();
    recordPerformanceSample(ddaSample, bresenhamSample);

    for (const auto& line : animatedLines) {
      const PixelPoint a = line.first;
      const PixelPoint b = line.second;
