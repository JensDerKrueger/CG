...
const float radius = 0.0006f;
const float colDist = 2 * radius;
const size_t particleCount = 20000;
Octree octree{ 0.02f, Vec3{0.0f,0.0f,0.0f}, 5, 6 };
...
void simulate(size_t particleCount)
{
	for (size_t i = 0; i < particleCount; ++i) {
		...
		std::this_thread::sleep_for(std::chrono::duration<double>(1.0 / simulationSpeed));
	}
}
...
int main(int argc, char** argv)
{
	...
	SimpleStaticParticleSystem simplePS(fixedParticles, 1);
...
#ifdef only2D
	Vec3 lookFromVec{ 0,0,cameraDistance };
#else
	Vec3 lookFromVec{ 0,cameraDistance,cameraDistance };
...
#else
		const Mat4 p{ Mat4::perspective(6.0f, dim.aspect(), 0.0001f, 1000.0f) };
		const Mat4 m{ Mat4::rotationY(25 * float(glfwGetTime()) / 2.0f) * Mat4::rotationX(10 * float(glfwGetTime()) / 2.0f) };
		simplePS.setPointSize(dim.height / 80.0f);
#endif
